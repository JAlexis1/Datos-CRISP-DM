...
class Standarization:
    ...
    def __verify_dtypes(self):
        ...
        for col in self.df.columns:
            # 1. Intentar convertir a numérico si parece número
            ...
            # 2. Intentar convertir a fecha si parece fecha
            if self.df[col].dtype == "object":
                try:
                    self.df[col] = pd.to_datetime(self.df[col], format="%Y-%m-%d %H:%M:%S")
                    continue
                except:
                    pass
    ...
    def __drop_categorical_columns(self):
        categorical_cols = [
            "Transaction_Type",
            "Device_Type",
            "Location",
    ...
    def __detected_outliers_zscore(self):
        ...
        upper_limit = self.df[self.df["zscore_amount"] <= threshold]["transaction_amount"].max()
        lower_limit = self.df[self.df["zscore_amount"] >= -threshold]["transaction_amount"].min()
        ...
        self.df.loc[self.df["zscore_amount"] > threshold, "transaction_amount_winsorized"] = upper_limit
        self.df.loc[self.df["zscore_amount"] < -threshold, "transaction_amount_winsorized"] = lower_limit

        min_outlier_value = outliers["transaction_amount"].min()
        print(f"→ Valores considerados atípicos (Z-score > 3):")
    ...
    def __replace_original_amount(self):
        ...
        self.df.drop(columns=["transaction_amount_winsorized", "zscore_amount"], inplace=True)


    def run(self):
        print("=== Iniciando estandarización del dataset ===")
        self.__drop_categorical_columns()
